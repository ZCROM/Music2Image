...
if __name__ == '__main__':
    ...
    # setup()
    #----------------musicCut------------------------------------------
    musicInputPath = r"./Music_input_data"
    files = os.listdir(musicInputPath)
    files = [musicInputPath + "\\" + f for f in files if f.endswith('.wav')]
    for indexFiles in range(len(files)):
        ...
        while StepTotalNum < nframes:
            ...
            FileName = "./musicCutResults/" + files[indexFiles][-6:-4] + "-" + str(haha + 1) + ".wav"
            # print(FileName)
            ...
            f = wave.open(FileName, "wb")  # write wav files
            # 配置声道数、量化位数和取样频率
